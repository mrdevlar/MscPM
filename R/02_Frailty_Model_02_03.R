...
model {
  for(i in 1:N){
      lifetime[i] ~ surv_dens(alp, gam, beta_err1 * x_err1[i] + beta_err2 * x_err2[i] + 
                  beta_repair * x_repair[i] + beta_kWh * x_kWh[i] + beta_weather * park_weather[i], Z[park[i]]);
  }
...
## Parameter Output
print(mf2m, pars = c("alp","gam", "Z", "beta_err1","beta_err2","beta_repair","beta_kWh","beta_weather") )
plot(mf2m, pars = c("alp","gam", "Z", "beta_err1","beta_err2","beta_repair","beta_kWh","beta_weather") )

## Check Traceplot
traceplot(mf2m, pars = c("alp","gam","Z", "beta_err1","beta_err2","beta_repair","beta_kWh","beta_weather") )
...
## Extract Samples
samp = extract(mf2m, pars = c("alp","gam","Z","beta_err1","beta_err2","beta_repair","beta_kWh","beta_weather") )
...
s_Z             = rnorm(s_n, mean = s_mean$Z,            sd = s_sd$Z)
